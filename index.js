(function (global, factory) {
    ...
    function update() {
      ...
      if (!['frequency', 'continuous', 'categorical'].includes(this.settings.colorBy.type)) {
        alert("[ ".concat(colorBy, " ] is not a valid option.  Please choose one of [ frequency ], [ continuous ], or [ categorical ].  Defaulting to  [ frequency ]."));
    ...
    function defineColor(value, colorScale) {
      var color = this.settings.colorBy.type !== 'frequency' || this.settings.eventChangeCountAesthetic !== 'size' ? d3.rgb(colorScale(value)).toString() : 'rgb(170,170,170)';
    ...
    function defineDatum(group, state, colorScale) {
      ...
      var aesthetic = this.settings.colorBy.type !== 'continuous' ? nStateChanges : state[this.settings.colorBy.variable]; // Define radius.
    ...
    */
    ...
    function data() {
      ...
      this.data.nested.forEach(function (d) {
        ...
        var destination = _this.settings.colorBy.type === 'categorical' && _this.settings.colorBy.stratify ? _this.metadata.event.find(function (event) {
      ...
      this.metadata.event.forEach(function (event) {
        ...
        if (event.foci) event.foci.forEach(function (focus) {
          ...
          focus.cumulative = _this.data.filter(function (d) {
            return d.event === event.value && d.category === focus.key && d.start_timepoint <= _this.settings.timepoint;
          }).length;
    ...
    function colorScale(metadata) {
      ...
      var domain = colorBy.type === 'frequency' ? d3.range(this.settings.nColors) : colorBy.type === 'continuous' ? d3.extent(this.data, function (d) {
        return d[colorBy.variable];
      }) : colorBy.type === 'categorical' ? metadata.strata.map(function (d) {
        return d.key;
      }) : null;
      var colorScale = colorBy.type === 'frequency' ? d3.scaleLinear().domain(domain).range(colorBy.mirror ? d3[this.settings.colorScheme][this.settings.nColors].reverse() : d3[this.settings.colorScheme][this.settings.nColors]).clamp(true) : colorBy.type === 'continuous' ? d3.scaleSequential(d3.interpolateRdYlGn).domain(domain) : colorBy.type === 'categorical' ? d3.scaleOrdinal().domain(domain).range(d3.schemeTableau10) : null; // Invert color scale.
    ...
    function text() {
      ...
      this.freqTable.tr.selectAll('td').data(function (event) {
        return [event.value, event.count, event.cumulative];
    ...
    function color(svg, legendDimensions) {
      ...
      var marks = svg.selectAll('rect.legend-mark').data(this.colorScale.range()).enter().append('rect').classed('legend-mark', true).attr('x', function (d, i) {
        return i * (legendDimensions[0] / _this.settings.nColors);
      }).attr('y', 0).attr('width', legendDimensions[0] / this.settings.nColors).attr('height', legendDimensions[1] / 3).attr('fill', function (d) {
        return d;
    ...
    function size(svg, legendDimensions) {
      ...
      var marks = svg.selectAll('circle.legend-mark').data(d3.range(this.settings.nColors)).join(this.settings.shape === 'circle' ? 'circle' : 'rect').classed('legend-mark', true).attr('fill', function (d) {
        return '#aaa';
      }).attr('fill-opacity', 0.5).attr('stroke', function (d) {
        return '#aaa';
      }).attr('stroke-opacity', 1);
      if (this.settings.shape === 'circle') marks.attr('cx', function (d, i) {
        return i * (legendDimensions[0] / _this.settings.nColors) + legendDimensions[0] / _this.settings.nColors / 2;
        ...
        return i * (legendDimensions[0] / _this.settings.nColors) + legendDimensions[0] / _this.settings.nColors / 2 - (i + _this.settings.minRadius);
      }).attr('y', function (d, i) {
    ...
    function both(svg, legendDimensions) {
      ...
      var marks = svg.selectAll('circle.legend-mark').data(this.colorScale.range()).join(this.settings.shape === 'circle' ? 'circle' : 'rect').classed('legend-mark', true).attr('fill', function (d) {
        return d;
      }).attr('fill-opacity', 0.5).attr('stroke', function (d) {
        return d;
      }).attr('stroke-opacity', 1);
    ...
    function makeLegend(type) {
      ...
      var lower = svg.append('text').attr('x', legendDimensions[0] / this.settings.nColors / 2).attr('y', legendDimensions[1] / 2 + 16).attr('text-anchor', 'middle').text('0'); // upper end of scale

      var upper = svg.append('text').attr('x', legendDimensions[0] - legendDimensions[0] / this.settings.nColors / 2).attr('y', legendDimensions[1] / 2 + 16).attr('text-anchor', 'middle').text("".concat(this.settings.nColors - 1, "+"));
      return {
        container: container,
        label: label,
        svg: svg,
    ...
    function addFreqTable() {
      ...
      freqTable.th = freqTable.thead.selectAll('th').data(['Participants', 'Events']).join('th').attr('colspan', function (d) {
        return d === 'Participants' ? 2 : null;
      }).style('text-align', 'right') //.style('font-weight', 'normal')
      .style('border-bottom', 'none').text(function (d) {
        return d;
      });
      freqTable.tbody = freqTable.table.append('tbody');
      this.data.freqTable = d3.merge(this.metadata.event.map(function (event) {
        var group = [event].concat(_toConsumableArray(event.foci));
        group.forEach(function (d) {
          d.group = event.value;
        });
        return group;
      })).filter(function (d) {
        return d.group !== _this.settings.eventCentral;
      });
      freqTable.tr = freqTable.tbody.selectAll('tr').data(this.data.freqTable).join('tr').style('font-size', function (d) {
      ...
      freqTable.tr.each(function (d, i) {
        ...
        var td = tr.selectAll('td').data(function (di) {
          return [di.value, di.count, di.cumulative];
        }).join('td').style('white-space', 'nowrap').style('overflow', 'hidden').style('text-overflow', 'ellipsis').style('border-top', d.group === d.value ? '1px solid #aaa' : null).style('text-align', function (di) {
          return typeof di === 'number' ? 'right' : 'left';
        }).style('padding-left', d.group !== d.value ? '2rem' : null).text(function (di) {
    ...
    function annotateFoci() {
      ...
      if (this.settings.colorBy.type === 'categorical' && this.settings.colorBy.stratify) {
        this.metadata.event.forEach(function (event) {
          ...
          ['background', 'foreground'].forEach(function (pos) {
            var text = event.fociLabels.selectAll("text.fdg-focus-annotation__".concat(pos)).data(event.foci).join('text').classed("fdg-focus-annotation__text fdg-focus-annotation__".concat(pos), true).attr('x', function (d) {
              return d.dx;
            }).attr('dx', function (d) {
    ...
    function nestData() {
      ...
      var nestedData = d3.nest().key(function (d) {
        ...
        var colorScale = _this.settings.colorBy.type === 'categorical' ? _this.metadata.strata.find(function (stratum) {
