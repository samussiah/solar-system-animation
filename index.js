(function (global, factory) {
    ...
    function coordinates(metadata) {
        ...
        metadata.event.forEach(function (event, i) {
            event.x =
                event.order === 0
                    ? centerX
                    : event.order * _this.settings.orbitRadius * Math.cos(thetaFactor(i) * theta) + //Math.cos((this.settings.nFoci - i) * theta) +
                      centerX;
            event.y =
                event.order === 0
                    ? centerY
                    : event.order * _this.settings.orbitRadius * Math.sin(thetaFactor(i) * theta) + //Math.sin((this.settings.nFoci - i) * theta) +
                      centerY;
    ...
    function updateText() {
        ...
        if (Array.isArray(this.settings.notes)) {
            if (
                ...
                this.settings.notes[this.settings.notesIndex].startTimepoint
            ) {
                ...
                this.settings.timepoint ===
                this.settings.notes[this.settings.notesIndex].stopTimepoint
            ) {
    ...
    var increment = function increment(arg) {
        ...
        this.containers.slider.attr('value', timepoint).attr(
            'title',
            'The animation is '
                .concat(d3.format('.1%')(timepoint / this.settings.duration), ' complete with ')
                .concat(this.settings.duration - timepoint, ' ')
                .concat(this.settings.timeUnit.split(' ')[0], ' to go.')
        );
    ...
    function forceDirectedGraph(data) {
        ...
        if (Array.isArray(fdg.settings.notes))
            fdg.settings.notesIndex = settings$1.notes.some(function (note) {
                return (
                ...
                ? fdg.settings.notes.findIndex(function (note) {
                      return (
                          note.startTimepoint <= settings$1.timepoint &&
