(function (global, factory) {
    ...
    function sidebar(main) {
        ...
        var timepoint = addElement('timepoint', progress)
            .classed('fdg-sidebar__label', true)
            .text(
                ''
        ...
        var timeRelative = addElement('time-relative', progress)
    ...
    function canvas(main) {
        ...
        var progressBar = addElement('progress-bar', animation).classed(
            'fdg-hidden',
            !this.settings.displayProgressBar
        );
        var progressTimepoint = addElement('progress-timepoint', animation).classed(
            'fdg-hidden',
            !this.settings.displayProgressBar
        ); // background SVG
    ...
    function eventMetadata() {
        ...
        this.metadata.event.forEach(function (event) {
            ...
            event.cells =
                _this.settings.freqTable.structure === 'vertical'
                    ? [event.label, event.countProportionFmt, event.cumulativeFmt]
                    : [event.label, event.countProportionFmt].concat(
                          _toConsumableArray(
                              event.foci
                                  ? event.foci.map(function (focus) {
                                        return focus.countProportionFmt;
                                    })
                                  : []
                          )
                ...
                event.foci.forEach(function (focus) {
                    ...
                    focus.cells =
                        _this.settings.freqTable.structure === 'vertical'
                            ...
                            : [focus.label, focus.countProportionFmt].concat(
                                  _toConsumableArray(
                                      focus.foci
                                          ? focus.foci.map(function (focus) {
                                                return focus.countProportionFmt;
                                            })
                                          : []
                                  )
        ...
        freqTable.forEach(function (d) {
            d.cells =
                _this.settings.freqTable.structure === 'vertical'
                    ...
                    : [d.label, d.countProportionFmt].concat(
                          _toConsumableArray(
                              d.foci
    ...
    function timeout(countdown) {
        ...
        var timeout = window.setTimeout(function () {
            ...
            _this.containers.timer.foreground
                .transition()
    ...
    function categoricallyReposition(text, label, eventCount) {
        ...
        if (this.settings.colorBy.type === 'categorical' && this.settings.colorBy.stratify) {
            ...
            label.attr('text-anchor', function (d) {
                return d.value === _this.settings.eventCentral ? 'start' : 'middle';
            });
