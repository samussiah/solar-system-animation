(function (global, factory) {
    ...
    function canvas(main) {
        ...
        var progressTimepoint = this.util
            ...
            .classed('fdg-hidden', !this.settings.displayProgressBar); // background SVG
                      ...
                      function (d, i) {
                          return (
                              (d.start_timepoint <= _this.settings.timepoint &&
                                  _this.settings.timepoint <= d.end_timepoint) || // first (and hopefully only) state that overlaps the current timepoint
                              (_this.settings.timepoint <= minTimepoint && i === 0) || // first state
                              (_this.settings.timepoint >= maxTimepoint && i === group.length - 1)
                          );
                      } // last state
                  );
        return state;
    ...
    function runSequence(sequence, event) {
        ...
        this.containers.sequence.html(
            'Sequence: '
                .concat(sequence.label, '<br><small>')
                .concat(start_orbit.label, ': ')
                .concat(sequence.event.key, '</small>')
        );
        ...
        if (this.sequence.event_index === 0)
            sequence.data = this.data
                .filter(function (d) {
                    return (
                        sequence.start_order <= d.event_order && d.event_order <= sequence.end_order
                    );
                })
                .map(function (d) {
                        ...
                        if (d === baseline) d.start_timepoint = 1;
                            ...
                            d.start_timepoint =
                                d.duration_cumulative < sequence.duration || !sequence.duration
                                    ...
                                    : sequence.duration; // Adjust end timepoint.
        ...
        var duration =
            d3.max(
                sequence.data.nested.filter(function (d) {
    ...
    function eventList() {
        ...
        if (
            ...
            inputs = container
                ...
                .attr('class', function (d) {
                    return 'fdg-button '.concat(
                        _this.settings.eventChangeCount.includes(d.key) ? 'current' : ''
                    );
                })
                .attr('title', function (d) {
                    return ''
            ...
            inputs.on('click', function (d) {
                ...
                this.title = ''
                    .concat(fdg.settings.eventChangeCount.includes(d.key) ? 'Remove' : 'Add', ' ')
                    .concat(d.key, ' ')
                    .concat(
    ...
    function init() {
        ...
        this.settings.removeCenterForce = true; // Start the timer.
