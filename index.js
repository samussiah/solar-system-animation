(function (global, factory) {
    ...
    function event() {
      ...
      var theta = 2 * Math.PI / (this.metadata.event.length - 2);
    ...
    function defineMetadata() {
      ...
      id.call(this); // Add additional metadata to event set.

      event.call(this); // Update settings that depend on data.

      dataDrivenSettings.call(this); // Define orbits.
    ...
    function eventList() {
      ...
      inputs.on('click', function (d) {
        ...
        this.title = "".concat(fdg.settings.eventChangeCount.includes(d.value) ? 'Remove' : 'Add', " ").concat(d.value, " ").concat(fdg.settings.eventChangeCount.includes(d.value) ? 'from' : 'to', " the list of events that control bubble ").concat(fdg.settings.eventChangeCountAesthetic === 'both' ? 'color and size' : fdg.settings.eventChangeCountAesthetic, "."); // Update legend label.

        fdg.legends.classed('fdg-hidden', fdg.settings.eventChangeCount.length === 0).selectAll('span.fdg-measure').text(fdg.settings.eventChangeCount.join(', ').replace(/, ([^,]*)$/, ', and $1')); // Recalculate radius and fill/stroke of points.
    ...
    function colorSizeToggle() {
      ...
      var inputs = container.selectAll('div').data(['color', 'size', 'both']).enter().append('div').attr('class', function (d) {
        ...
        return "Quantify the number of times ".concat('this.settings.eventsToCount', " have occured by ", d !== 'both' ? d : 'color and size');
      }).text(function (d) {
        return d;
      });
      inputs.on('click', function (d) {
    ...
    function color$1() {
      ...
      this.colorLegend = this.legends.append('div').classed('fdg-legend fdg-legend--color', true).classed('fdg-hidden', this.settings.eventChangeCountAesthetic !== 'color' || this.settings.eventChangeCount.length === 0);
      var legendDimensions = [200, 50];
      this.colorLegend.append('div').classed('fdg-legend__label', true).style('width', legendDimensions[0] + 'px').html("Number of <span class = \"fdg-measure\">".concat(this.settings.eventChangeCount.join(', ').replace(/, ([^,]*)$/, ', and $1'), "</span> events"));
    ...
    function size() {
      ...
      this.sizeLegend = this.legends.append('div').classed('fdg-legend fdg-legend--size', true).classed('fdg-hidden', this.settings.eventChangeCountAesthetic !== 'size' || this.settings.eventChangeCount.length === 0);
      var legendDimensions = [200, 50];
      this.sizeLegend.append('div').classed('fdg-legend__label', true).style('width', legendDimensions[0] + 'px').html("Number of <span class = \"fdg-measure\">".concat(this.settings.eventChangeCount.join(', ').replace(/, ([^,]*)$/, ', and $1'), "</span> events"));
    ...
    function both() {
      ...
      this.bothLegend = this.legends.append('div').classed('fdg-legend fdg-legend--both', true).classed('fdg-hidden', this.settings.eventChangeCountAesthetic !== 'both' || this.settings.eventChangeCount.length === 0);
      var legendDimensions = [200, 50];
      this.bothLegend.append('div').classed('fdg-legend__label', true).style('width', legendDimensions[0] + 'px').html("Number of <span class = \"fdg-measure\">".concat(this.settings.eventChangeCount.join(', ').replace(/, ([^,]*)$/, ', and $1'), "</span> events"));
    ...
    function collide(alpha) {
      ...
      return function (d) {
        ...
        var r = fdg.settings.eventChangeCountAesthetic !== 'color' ? d.r + fdg.settings.maxRadius + fdg.settings.padding : d.r + fdg.settings.minRadius + fdg.settings.padding; // dimensions of current node expressed as a square
        ...
        quadtree.visit(function (quad, x1, y1, x2, y2) {
          ...
          if (quad.point && quad.point !== d) {
            var x = d.x - quad.point.x; // difference in x coordinate between current node and nearby node

            var y = d.y - quad.point.y; // difference in y coordinate between current node and nearby node

            var l = Math.sqrt(x * x + y * y); // Euclidean distance between current node and nearby node

            var _r = d.r + quad.point.r + (d.currentEvent.event !== quad.point.currentEvent.event) * fdg.settings.padding; // minimum distance allowed between current node and nearby node in the quadtree
    ...
    function tick(e) {
      ...
      this.circles.each(collide.call(this, 0.8)).attr('cx', function (d) {
        return d.x;
      }).attr('cy', function (d) {
        return d.y;
      }).attr('r', function (d) {
