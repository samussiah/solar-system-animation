(function (global, factory) {
    ...
    function tick(e) {
      ...
      this.data.nested.forEach(function (d, i) {
        ...
        d.y += (currentEvent.y - d.y) * k; //d.color = this.settings.colorScale(
        //    d3.sum(d.eventTypes.filter(eventType => eventType.label !== this.settings.centerEventType), eventType => eventType.count)
        //);
      });
      this.circles.each(collide.call(this, 0.5)).style('fill', function (d) {
        return d.color;
    ...
    function addCircles() {
      var circles = this.svg.selectAll('circle').data(this.data.nested).enter().append('circle').attr('r', function (d) {
        return d.r;
    ...
    function addTimer() {
      ...
      this.data.nested.forEach(function (d) {
        var currentEvent = d.currentEvent.event;
        ...
        if (d.next_move_time === _this.settings.timepoint) {
          ...
          _this.eventTypes.find(function (eventType) {
            return eventType.label === currentEvent;
          }).count -= 1; // Move on to next activity

          d.currentEvent = d.sched[curr_moves];
          var eventType = d.eventTypes.find(function (eventType) {
            return eventType.label === d.currentEvent.event;
          });
          eventType.count += 1; // Add to new activity count
