(function (global, factory) {
    ...
    function layout() {
      ...
      var controls = addElement('controls', main).classed('fdg-hidden', this.settings.hideControls); // sidebar to the left
      ...
      var slider = addElement('slider', timing, 'input').attr('type', 'range').attr('min', 0).attr('value', 0).property('disabled', true).attr('title', "The animation is ".concat(d3.format('.1%')(this.settings.timepoint / this.settings.duration), " complete with ").concat(this.settings.duration, " ").concat(this.settings.timeUnit.split(' ')[0], " to go.")); // TODO: make slider into a control - need to figure out a new way to count up the state changes up to the selected timepoint
    ...
    function getState(group, index) {
      ...
      var state = index ? group[index] : group.find(function (d, i) {
    ...
    function text() {
      var text = ['Each bubble in this animation represents an individual.', 'As individuals experience events and change states, their bubble gravitates toward the focus representing that event.', 'The number of state changes dictates the color and/or size of the bubbles.', 'Use the controls above to interact with and alter the animation.'];
    ...
    function fadeIn(selection) {
      selection.style('opacity', 0).transition().duration(1000).style('opacity', 1).on('end', fadeOut);
    }
    ...
    var increment = function increment(arg) {
      ...
      this.settings.timepoint += !!arg; // Ensure the timepoint does not pass the total duration - off-by-one error?

      var timepoint = Math.min(this.settings.timepoint, this.settings.duration);
      this.containers.slider.attr('value', timepoint).attr('title', "The animation is ".concat(d3.format('.1%')(timepoint / this.settings.duration), " complete with ").concat(this.settings.duration - timepoint, " ").concat(this.settings.timeUnit.split(' ')[0], " to go."));
      this.controls.timepoint.inputs.property('value', timepoint);
    ...
    function nestData() {
      ...
      var nestedData = d3.nest().key(function (d) {
        ...
        var state = getState.call(_this, group, 0); // Count state changes.
    ...
    function forceDirectedGraph(data) {
      ...
      if (Array.isArray(fdg.settings.notes)) fdg.settings.notesIndex = settings$1.notes.some(function (note) {
        return note.startTimepoint <= fdg.settings.timepoint && fdg.settings.timepoint <= note.endTimepoint;
      }) ? fdg.settings.notes.findIndex(function (note) {
        return note.startTimepoint <= settings$1.timepoint && fdg.settings.timepoint <= note.stopTimepoint;
      }) : 0;
