(function (global, factory) {
    ...
    function canvas(main) {
        ...
        sequenceOverlay.background.sequence = this.util
            .addElement(
                'sequence-overlay__background__sequence',
                sequenceOverlay.background,
                'tspan'
            )
        ...
        sequenceOverlay.foreground.sequence = this.util
            .addElement(
                'sequence-overlay__foreground__sequence',
                sequenceOverlay.foreground,
                'tspan'
            )
            .classed('fdg-focus-annotation__label', true)
            .attr('x', 0)
        ...
        var modalContainer = this.util
            .addElement('modal', container)
            .attr('class', function (d) {
                return 'fdg-modal '
                    .concat(
                        _this.settings.modalPosition
                            .split('-')
                    ...
                    .concat(_this.settings.modalPosition);
            })
    ...
    function coordinates(metadata) {
        ...
        metadata.event.forEach(function (event, i) {
            ...
            event.ry =
                _this.settings.orbitShape === 'circle' || i === 0
                    ? event.radius
                    : event.radius - (event.radius * orbit) / (orbit + 2); // Define angle of event focus.
    ...
    function draw(worker, color) {
        ...
        worker.onmessage = function (event) {
            ...
            var g = main.layout.svgBackground
                .insert('g', ':first-child')
                .classed('fdg-static '.concat(className), true)
                .attr('transform', 'translate('.concat(main.settings.width.sidebar, ',0)')); // translate to the central focus

            if (main.settings.staticLayout == 'radial')
                g.attr(
                    'transform',
                    'translate('
                        .concat(main.settings.width.sidebar + main.settings.orbitRadius / 2, ',')
    ...
    function nestedData(data) {
        ...
        data.nested.forEach(function (d, i) {
            ...
            d.value.distanceFromFocus = Math.sqrt(
                Math.pow(d.x - d.value.coordinates.x, 2) + Math.pow(d.y - d.value.coordinates.y, 2)
            );
            ...
            d.value.transitTime++;
            var transitThreshold =
                500 / Math.ceil(Math.sqrt(_this.settings.speeds[_this.settings.speed]));

            if (
                _this.settings.enforceFocusVicinity === false ||
                d.value.inVicinityOfFocus ||
                ((_d$value$statePreviou = d.value.statePrevious) === null ||
                _d$value$statePreviou === void 0
                    ? void 0
                    : _d$value$statePreviou.event) === _this.settings.eventCentral ||
    ...
    function formatValues(d) {
        ...
        var eventPercent = d3.format('.1%')(
            d.freqs.eventDenominator > 0 ? d.freqs.eventProportion : 0
        );
    ...
    function defineCellValues(d) {
        ...
        if (this.settings.freqTable.structure === 'vertical') {
            ...
            if (d.foci)
                d.foci.forEach(function (focus) {
                    if (_this.settings.freqTable.countType === 'id')
                        cells.push({
    ...
    function resize() {
        ...
        this.layout.timer.g.attr(
            'transform',
            'translate('
                .concat(this.layout.timer.width / 2, ',')
                .concat(this.layout.timer.width / 2, ')')
        );
        ...
        this.layout.svgBackground
            .attr('width', this.settings.width.main)
            .attr('height', this.settings.height.main)
            .style('left', -this.settings.width.sidebar); // canvas

        this.layout.canvas
            .attr('width', this.settings.width.canvas)
            .attr('height', this.settings.height.main); // SVG
        ...
        this.layout.svgBackground
            ...
            .attr('transform', 'translate('.concat(this.settings.width.sidebar, ',0)')); // focus coordinates
    ...
    function display() {
        ...
        inputs.on('click', function (d) {
            ...
            main.controls.display.inputs
                .html("Controls <span class = 'fdg-expand'>".concat(d.symbol, '</span>'))
                .attr('title', function (d) {
                    return d.state ? 'Hide controls.' : 'Display controls.';
                });
    ...
    function structure(data) {
        ...
        var grouped = d3
            ...
            .rollup(function (group) {
                ...
                var noStateChange =
                    group.length === 1 && group[0].event === _this.settings.eventCentral; // state-level values - calculated once per timepoint
    ...
    function counts() {
        ...
        if (this.settings.eventCount)
            this.focusAnnotations
                ...
                .text(function (d) {
                    return _this.settings.eventCountType === 'cumulative-event'
        ...
        this.layout.freqTable.tr.each(function (d) {
            ...
            tr.selectAll('td')
                ...
                .style('background', function (di, i) {
                    return di.key === 'id' && main.settings.freqTable.bars
                        ? 'linear-gradient(to right, var(--background-darkest) 0, var(--background-darkest) '
    ...
    function text(data) {
        ...
        if (this.settings.annotations && Array.isArray(this.settings.annotations))
            this.customAnnotations
                .filter(function (d) {
                    return (
                        d.timepoint <= main.settings.timepoint &&
        ...
        if (this.settings.timepoint < this.settings.duration) {
            if (
                ...
                this.layout.sequenceOverlay.background.sequence
                    ...
                    .style('opacity', 0)
                    .on('end', function () {
                        _this.layout.sequenceOverlay.background.sequence
                            .html(_this.currEvent.orbitLabel || _this.currEvent.key)
                            .style('opacity', 0)
                            .transition()
                            .duration(_this.settings.modalSpeed / 5)
                            .style('opacity', 1);
                    });
                this.layout.sequenceOverlay.foreground.sequence
                    ...
                    .style('opacity', 0)
                    .on('end', function () {
                        _this.layout.sequenceOverlay.foreground.sequence
                            .html(_this.currEvent.orbitLabel || _this.currEvent.key)
                            .style('opacity', 0)
                            .transition()
                            .duration(_this.settings.modalSpeed / 5)
            ...
            if (this.currEvent.orbitLabel && this.currEvent.key) {
                this.layout.sequenceOverlay.background.event
                    ...
                    .style('opacity', 0)
                    .on('end', function () {
                        _this.layout.sequenceOverlay.background.event
                            .html(_this.currEvent.key)
                            .style('opacity', 0)
                            .transition()
                            .duration(_this.settings.modalSpeed / 5)
                            .style('opacity', 1);
                ...
                this.layout.sequenceOverlay.foreground.event
                    ...
                    .style('opacity', 0)
                    .on('end', function () {
                        _this.layout.sequenceOverlay.foreground.event
                            .html(_this.currEvent.key)
                            .style('opacity', 0)
                            .transition()
                            .duration(_this.settings.modalSpeed / 5)
    ...
    function timeoutBetweenStates() {
        ...
        if (this.settings.stateChange === 'ordered') {
            ...
            this.currEvent = this.metadata.event.find(function (event) {
                return (
                    event.start_timepoint <= _this.settings.timepoint &&
                    _this.settings.timepoint <= event.end_timepoint
                );
            });

            if (
                this.settings.timepoint ===
                ((_this$currEvent = this.currEvent) === null || _this$currEvent === void 0
                    ? void 0
                    : _this$currEvent.start_timepoint)
            ) {
                this.interval.stop();
    ...
    function orbits() {
        ...
        this.orbits.each(function (d) {
            ...
            d.rAdj = isNaN(diff)
                ? d.rAdjPrev
                : d.rAdjPrev + Math.min(diff / 10, main.settings.orbitRadius / 100); //d.change = d3.sum(d.values, (di) => di.change);
            //if (d.change > 0) {
            //if (!isNaN(d.rAdj))
        ...
        switch (true) {
            ...
            case /controls/i.test(this.modalText):
                if (this.settings.hideControls === false)
                    emphasizeComponent.call(
                        this,
                        this.layout.controlsContainer.classed(
                            'fdg-hidden',
    ...
    function resetAnimation(data) {
        ...
        if (this.settings.annotations && Array.isArray(this.settings.annotations))
            this.customAnnotations.attr('opacity', function (d) {
    ...
    function runNextSequence() {
        ...
        if (
            ...
            else this.sequence.event = getNextEventInSequence.call(this); // Run sequence.
    ...
    var increment = function increment(data, _increment) {
        ...
        if (
            ...
            this.interval = startInterval.call(
                this,
                this.sequence ? this.sequence.data : this.data
            );
        } // Update focus label.
        ...
        if (
            ...
            var focusAnnotation = this.focusAnnotations
                .filter(function (d) {
        ...
        if (this.settings.timepoint <= this.settings.duration) update$1.call(this, data);
    ...
    function runSequence() {
        ...
        this.sequence.backgroundEvent = fadeIn.call(
            ...
            ''
                .concat(
                    this.settings.individualUnit.replace(/^(.)/, function (_char) {
                        return _char.toUpperCase();
                    }),
                    's: '
                )
                .concat(this.sequence.event.key)
        );
        this.sequence.foregroundEvent = fadeIn.call(
            ...
            ''
                .concat(
                    this.settings.individualUnit.replace(/^(.)/, function (_char2) {
        ...
        if (this.sequence.eventIndex === 0)
            this.sequence.data = this.data
                .filter(function (d) {
                    return (
                        _this.sequence.start_order <= d.event_order &&
                        d.event_order <= _this.sequence.end_order
                    );
                })
                .map(function (d) {
            ...
            d3.nest()
                ...
                .rollup(function (group) {
                    ...
                    group.forEach(function (d, i) {
                        ...
                        if (
                            !!_this.sequence.duration &&
                            d.start_timepoint > _this.sequence.duration
                        )
                            d.start_timepoint = _this.sequence.duration; // Set end timepoint to sequence duration if end timepoint is greater than sequence duration.

                        if (!!_this.sequence.duration && d.end_timepoint > _this.sequence.duration)
                            d.end_timepoint = _this.sequence.duration;
                    });
                    return group;
                })
        ...
        var duration =
            d3.max(
                this.sequence.data.nested.filter(function (d) {
        ...
        this.settings.duration = this.sequence.duration
            ...
            : duration; // Re-define force simulation.
    ...
    function sequences() {
        ...
        if (!!this.settings.sequences) {
            var main = this;
            var container = this.util
                .addElement('sequences', this.layout.controls)
                .classed('fdg-control fdg-control--sequences', true);
            var inputs = container
                .selectAll('div')
                .data([this].concat(_toConsumableArray(this.settings.sequences)))
                .join('div')
                .classed('fdg-button fdg-button--sequence', true)
                .attr('title', function (d) {
                    return 'View '.concat(
                        d !== _this ? 'sequence '.concat(d.label) : 'full animation',
                        '.'
                    );
                })
                .text(function (d, i) {
    ...
    function speed() {
        ...
        var inputs = container
            ...
            .attr('class', function (d) {
                return 'fdg-button '
                    .concat(d.label, ' ')
                    .concat(d.label === _this.settings.speed ? 'fdg-button--current' : '');
            })
            .attr('title', function (d) {
    ...
    function reset$1() {
        ...
        inputs.on('click', function () {
            ...
            resetAnimation.call(
                _this,
                _this.settings.animationTrack === 'sequence' ? _this.sequence.data : _this.data
            );
            if (_this.forceSimulation) _this.forceSimulation.stop(); // Restart force simulation.

            _this.forceSimulation = addForceSimulation.call(
                _this,
                _this.settings.animationTrack === 'sequence' ? _this.sequence.data : _this.data
            ); // Start the animation.

            if (_this.settings.playPause === 'play')
                _this.timeout = d3.timeout(
                    function () {
                        // Run first sequence.
                        if (_this.settings.animationTrack === 'sequence') runSequence.call(_this);
                        // calls startInterval
                        // Run full animation.
            ...
            inputs = container
                ...
                .attr('class', function (d) {
                    return 'fdg-button '.concat(
                        _this.settings.eventChangeCount.includes(d.key) ? 'fdg-button--current' : ''
                    );
                })
                .attr('title', function (d) {
                    return ''
                        .concat(
                            _this.settings.eventChangeCount.includes(d.key) ? 'Remove' : 'Add',
                            ' '
                        )
                        .concat(d.key, ' ')
                        .concat(
                            _this.settings.eventChangeCount.includes(d.key) ? 'from' : 'to',
            ...
            inputs.on('click', function (d) {
                ...
                if (main.settings.eventChangeCount.includes(this.textContent))
                    main.settings.eventChangeCount.splice(
                        main.settings.eventChangeCount.findIndex(function (event) {
                            return event === _this2.textContent;
                        }),
                        1
                    );
                else main.settings.eventChangeCount.push(this.textContent); // Update tooltip.

                this.title = ''
                    .concat(main.settings.eventChangeCount.includes(d.key) ? 'Remove' : 'Add', ' ')
                    .concat(d.key, ' ')
                    .concat(
    ...
    function makeLegend(type) {
        ...
        var label = container
            ...
            .html(
                'Number of events' //`Number of <span class = "fdg-measure">${this.util.csv(
        ...
        var svg = container
            ...
            .attr(
                'transform',
                'translate('.concat(this.legends.svgWidth / 2 - this.legends.radius - 6, ',0)')
            ); // marks
        ...
        var upper = svg
            ...
            .attr('y', legendDimensions[1] / 2 + 16)
            .attr('text-anchor', 'middle')
            .html(''.concat(this.settings.colorBy.nColors - 1, '+'));
        return container;
    }
    ...
    function legendTable(label, domain) {
        ...
        container.symbols = container.rows
            ...
            .append('g')
            .attr('transform', 'translate(0,2)'); // Add labels to second cell of each row.

        container.labels = container.rows
            .append('td') //.style('padding-left', `${this.legends.svgWidth / 2 - this.legends.radius}px`)
            .html(function (d) {
                return d;
    ...
    function categorical() {
        ...
        var container = legendTable.call(
            this,
    ...
    function color$2() {
        ...
        if (this.settings.colorify) {
            ...
            container.classed(
                'fdg-legend fdg-legend--color fdg-legend--'.concat(this.settings.colorBy.type),
                true
            );
        }
    ...
    function size$2() {
        ...
        if (this.settings.sizify && this.settings.colorBy.type !== 'frequency') {
            container = makeLegend
                .call(this, 'size')
                .classed(
                    'fdg-legend fdg-legend--size fdg-legend--'.concat(this.settings.sizeBy.type),
                    true
                );
        }
    ...
    function shape$1() {
        ...
        if (this.settings.shapify) {
            container = legendTable
                ...
                .classed(
                    'fdg-legend fdg-legend--shape fdg-legend--'.concat(this.settings.shapeBy.type),
                    true
                ); // Draw symbols
            ...
            container.symbols.each(function (value, i) {
                var shape = shapes[main.scales.shape(value)]
    ...
    function addFreqTable() {
        ...
        freqTable.title = this.util
            ...
            .text(
                this.settings.freqTable.structure === 'vertical'
                    ...
                    ? 'Number of '.concat(this.settings.individualLabel)
                    : 'Number of '.concat(this.settings.eventLabel)
            );
        ...
        freqTable.thead = freqTable.table
            ...
            .classed('fdg-hidden', this.settings.freqTable.header === false); // Add column headers.
        ...
        freqTable.th = freqTable.thead
            ...
            .data(
                this.settings.freqTable.structure === 'vertical'
                    ...
                    : ['', 'Total'].concat(
                          _toConsumableArray(
                              this.metadata.strata.map(function (stratum) {
                                  return stratum.key;
                              })
                          )
                      )
            ...
            .text(function (d) {
                return d === 'label'
                    ? ''
                    : d === 'id'
                    ? _this.settings.individualLabel
                    : d === 'event'
                    ? _this.settings.eventLabel
                    : d;
        ...
        freqTable.tr.each(function (d, i) {
            ...
            var td = tr
                ...
                .attr('class', function (di, i) {
                    return main.settings.freqTable.structure === 'vertical'
                        ? 'fdg-freq-table__td--'.concat(di.key)
                        : 'fdg-freq-table__td--'.concat(di.key === 'label' ? 'label' : 'freq');
                })
                .classed('fdg-freq-table__td', true)
                .style('background', function (di, i) {
                    return i === 1 && main.settings.freqTable.bars
                        ? 'linear-gradient(to right, var(--background-darkest) 0, var(--background-darkest) '
    ...
    function addOrbits() {
        var g = this.layout.svgBackground
            .append('g')
            .classed('fdg-g fdg-g--orbits', true)
            .attr('transform', 'translate('.concat(this.settings.width.sidebar, ',0)'));
        var shadows = g
            .append('defs')
            .selectAll('filter')
    ...
    function getRelative$1(d) {
        var centered =
            d.key === this.settings.eventCentral ||
            this.settings.focusOffset === 'none' ||
            (this.settings.focusOffset === 'heuristic' && isCenter$1.call(this, d));
        var above =
            this.settings.focusOffset === 'above' ||
            (this.settings.focusOffset === 'heuristic' &&
                isLessThanCenter$1.call(this, d) === true);
        var below =
            this.settings.focusOffset === 'below' ||
            (this.settings.focusOffset === 'heuristic' &&
                isLessThanCenter$1.call(this, d) === false);
        return centered
            ? 0
            : above
    ...
    function addLabel(text) {
        ...
        var label = text
            .append('tspan')
            .classed('fdg-focus-annotation__label', true)
            .attr('x', 0) //.attr('text-anchor', (d) => getTextAnchor.call(this, d))
            .attr('text-anchor', 'middle')
    ...
    function addEventCount(text) {
        var eventCount = text
            ...
            .attr('x', 0)
            .attr('dy', '1.3em') //.attr('text-anchor', (d) => getTextAnchor.call(this, d));
            .attr('text-anchor', 'middle')
            .attr('alignment-baseline', 'middle')
    ...
    function addFocusAnnotations() {
        ...
        var fociLabels = this.layout.focusAnnotations
        ...
        ['background', 'foreground'].forEach(function (pos) {
            ...
            texts.each(function (d, i) {
                ...
                text.attr('transform', function (d) {
                    return 'translate('
                        .concat(getRelative.call(main, d), ',')
                        .concat(getRelative$1.call(main, d), ')');
                });
            });
        ...
        if (this.settings.colorBy.type === 'categorical' && this.settings.colorBy.stratify) {
            this.metadata.event.forEach(function (event) {
                event.fociLabels = _this.layout.focusAnnotations
                    .append('g')
                    .classed('fdg-focus-annotation fdg-focus-annotation--categorical', true);
                ['background', 'foreground'].forEach(function (pos) {
                    var texts = event.fociLabels
                        .selectAll('text.fdg-focus-annotation__'.concat(pos))
                        .data(event.foci)
                        .join('text')
                        .classed(
                            'fdg-focus-annotation__event-count fdg-focus-annotation__text fdg-focus-annotation__'.concat(
                                pos
                            ),
                            true
                        )
    ...
    function addCustomAnnotations() {
        ...
        if (this.settings.annotations && Array.isArray(this.settings.annotations)) {
            ...
            annotations = this.layout.customAnnotations
    ...
    function layout() {
        ...
        for (var prop in this.settings.root) {
            document
                .querySelector(':root')
                .style.setProperty('--'.concat(prop), this.settings.root[prop], 'important');
    ...
    function updateIdDependentSettings(metadata) {
        ...
        this.settings.staticRadius =
            this.settings.staticRadius || Math.max(3000 / metadata.id.length, 1);
    ...
    function event() {
        ...
        var nest = d3
            ...
            .rollup(function (group) {
                ...
                var position = datum.hasOwnProperty('event_position')
    ...
    function orbit(metadata) {
        ...
        nest.forEach(function (orbit, i) {
            orbit.label =
                Array.isArray(_this.settings.orbitLabels) &&
                _this.settings.orbitLabels.length === nest.length
                    ? _this.settings.orbitLabels[i]
                    : 'Orbit '.concat(i + 1);
            orbit.values
                .sort(function (a, b) {
    ...
    function strata(metadata) {
        ...
        if (this.settings.stratify) {
            nest = d3
                ...
                .entries(this.data)
                .sort(function (a, b) {
                    var aOrder = Array.isArray(_this.settings.colorBy.order)
                        ? _this.settings.colorBy.order.indexOf(a.key)
                        : null;
                    var bOrder = Array.isArray(_this.settings.colorBy.order)
                        ? _this.settings.colorBy.order.indexOf(b.key)
                        : null;
    ...
    function shape$2(metadata) {
        ...
        if (this.settings.shapify) {
            nest = d3
                ...
                .entries(this.data)
                .sort(function (a, b) {
                    var aOrder = Array.isArray(_this.settings.shapeBy.order)
                        ? _this.settings.shapeBy.order.indexOf(a.key)
                        : null;
                    var bOrder = Array.isArray(_this.settings.shapeBy.order)
                        ? _this.settings.shapeBy.order.indexOf(b.key)
                        : null;
                    var orderSort = aOrder - bOrder;
                    var alphaSort = a.key < b.key ? -1 : 1;
                    return orderSort ? orderSort : alphaSort;
            ...
            metadata.event.map(function (event) {
                ...
                var rowGroup =
                    _this.settings.colorBy.type === 'categorical'
                        ...
                        : [event]; // For each row calculate numerators, denominators, and proportions.
                ...
                .rollup(function (group) {
                    ...
                    group.forEach(function (d, i) {
                        ...
                        var prevState = _this.metadata.event.find(function (event) {
                            ...
                            return (
                                event.key ===
                                ((_group = group[i - 1]) === null || _group === void 0
                                    ? void 0
                                    : _group.event)
                            );
                        });
                        ...
                        if (i === 0) {
                            ...
                            d.end_timepoint =
                                d.start_timepoint +
                                currState.allIds.findIndex(function (id) {
                                    return id === d.id;
                                });
                        } else {
                            d.start_timepoint =
                                prevState.allIds.findIndex(function (id) {
                                    return id === d.id;
                                }) +
                                prevState.start_timepoint +
                                1;
                            d.end_timepoint =
                                currState.allIds.findIndex(function (id) {
                                    return id === d.id;
