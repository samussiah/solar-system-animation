(function (global, factory) {
    ...
    function addForceSimulation(data) {
      ...
      })).alphaDecay(0.01).velocityDecay(0.9).force('center', d3.forceCenter(this.settings.orbitRadius / 2, this.settings.height.main / 2)) // cleared after first interval
    ...
    function legendTable(label, domain) {
      ...
      container.rows = container.table.append('tbody').selectAll('tr').data(domain).join('tr'); // Add labels to second cell of each row.

      container.labels = container.rows.append('td').style('padding-left', "".concat(this.legends.svgWidth / 2 - this.legends.radius, "px")).html(function (d) {
        return d;
      }); // Add SVG elements to first cell of each row.

      container.symbols = container.rows.append('td').append('svg').style('width', this.legends.svgWidth).style('height', this.legends.svgHeight).append('g').attr('transform', 'translate(0,2)'); // Add counts to third cell of each row.
