(function (global, factory) {
    ...
    function sidebar(main) {
      ...
      var events = addElement('events', sidebar).text(this.settings.eventLabel);
      var legends = addElement('legends', sidebar);
      var progress = addElement('progress', sidebar);
      var timepoint = addElement('timepoint', progress).classed('fdg-sidebar__label', true);
    ...
    function eventMetadata() {
      ...
      this.metadata.event.forEach(function (event) {
        ...
        event.countProportionFmt = "".concat(event.numeratorFmt, " (").concat(event.proportionFmt, ")");
    ...
    function freqTable(metadata) {
      ...
      var freqTable = d3.merge(metadata.event.map(function (event) {
        ...
        rowGroup.forEach(function (d) {
          ...
          d.denominator = d.label !== d.state ? d.nParticipants : ['current-id', 'cumulative-id'].includes(_this.settings.eventCountType) ? metadata.id.length : _this.settings.eventChangeType === 'cumulative-event' ? event.nEvents : console.warn('Unable to determine [ event.denominator ] in [ eventMetadata ].');
    ...
    function progress() {
      // Update timepoint.
      this.containers.timepoint.text("".concat(this.settings.timepoint, " ").concat(this.settings.timeUnit)); // Update timer.
      ...
      this.containers.progressDay.style('right', "".concat(100 - this.settings.progress * 100, "%")).text("Day ".concat(this.settings.timepoint));
    }
    ...
    function legends() {
      ...
      this.containers.legends.selectAll('.fdg-legend--shape').selectAll('text').text(function (d) {
        return "".concat(d, " (n=").concat(shapeCounts.find(function (di) {
          return di.key === d;
        }).value, ")");
      });
    }
    ...
    function freqTable$1() {
      ...
      this.freqTable.tr.each(function (d) {
        ...
        tr.selectAll('td').data(d.cells).join('td').style('background', function (di, i) {
          return i === 1 ? "linear-gradient(to right, var(--background-darkest) 0, var(--background-darkest) ".concat(relativeProportionFmt, ", transparent ").concat(relativeProportionFmt, ")") : null;
        }).text(function (d) {
          return d;
        });
      });
    ...
    function categorical() {
      ...
      container.append('div').classed('fdg-sidebar__label fdg-legend__label', true).text(this.settings.colorBy.label);
    ...
    function makeLegend(type) {
      ...
      var lower = svg.append('text').attr('x', legendDimensions[0] / this.settings.colorBy.nColors / 2).attr('y', legendDimensions[1] / 2 + 16).attr('text-anchor', 'middle').text('0'); // upper end of scale

      var upper = svg.append('text').attr('x', legendDimensions[0] - legendDimensions[0] / this.settings.colorBy.nColors / 2).attr('y', legendDimensions[1] / 2 + 16).attr('text-anchor', 'middle').text("".concat(this.settings.colorBy.nColors - 1, "+"));
      return {
        container: container,
        label: label,
        svg: svg,
    ...
    function circle(legendItem, i, spacing, radius) {
      legendItem.append('circle').attr('cx', spacing).attr('cy', i * spacing + 10).attr('r', radius - 1).attr('fill', 'none').attr('stroke', '#444');
    }

    function square(legendItem, i, spacing, radius) {
      legendItem.append('rect').attr('x', spacing - radius + 1.5).attr('y', i * spacing + radius / 2 + 1).attr('width', radius * 1.5).attr('height', radius * 1.5).attr('fill', 'none').attr('stroke', '#444');
    }
    ...
    function triangle(legendItem, i, spacing, radius) {
      ...
      legendItem.append('polygon').attr('points', [top, left, right]).attr('fill', 'none').attr('stroke', '#444');
    }

    function diamond(legendItem, i, spacing, radius) {
      var x = spacing;
      var y = i * spacing;
      legendItem.append('rect').attr('transform', "rotate(45 ".concat(x, " ").concat(y, ")")).attr('x', x).attr('y', y).attr('width', radius * 1.5).attr('height', radius * 1.5).attr('fill', 'none').attr('stroke', '#444');
    }
    ...
    function star(legendItem, i, spacing, radius) {
      ...
      legendItem.append('polygon').attr('points', coordinates).attr('fill', 'none').attr('stroke', '#444');
    }
    ...
    function shape$2() {
      ...
      container.append('div').classed('fdg-sidebar__label fdg-legend__label', true).text(this.settings.shapeBy.label);
    ...
    function addFreqTable() {
      ...
      freqTable.th = freqTable.thead.selectAll('th').data(['', 'Participants', 'Events']).join('th').attr('class', function (d, i) {
        return "fdg-freq-table__th--".concat(i === 0 ? 'label' : i === 1 ? 'participant' : i === 2 ? 'event' : i);
      }).classed('fdg-freq-table__th', true).text(function (d) {
        return d;
      });
      freqTable.th.filter(function (d) {
      ...
      freqTable.tr = freqTable.tbody.selectAll('tr').data(this.data.freqTable.filter(function (d) {
        return !(_this.settings.eventCentralInFreqTable === false && d.state === _this.settings.eventCentral);
      ...
      freqTable.tr.each(function (d, i) {
        ...
        var td = tr.selectAll('td').data(function (di) {
          ...
          return "fdg-freq-table__td--".concat(i === 0 ? 'label' : i === 1 ? 'participant' : i === 2 ? 'event' : i);
        }).classed('fdg-freq-table__td', true).style('background', function (di, i) {
          return i === 1 ? "linear-gradient(to right, var(--background-darkest) 0, var(--background-darkest) ".concat(d.proportionFmt, ", transparent ").concat(d.proportionFmt, ")") : null;
    ...
    function dataDrivenLayout() {
      ...
      this.containers.timepoint.text("".concat(this.settings.timepoint, " ").concat(this.settings.timeUnit));
