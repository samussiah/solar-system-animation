(function (global, factory) {
    ...
    function event() {
      ...
      var theta = 2 * Math.PI / (this.metadata.event.length - 1);
    ...
    function step() {
      ...
      inputs.on('click', function () {
        // Pause simulation.
        if (_this.settings.playPause !== 'pause') toggle.call(_this); // Update timepoint.
        // Increment the timepoint.

        _this.settings.timepoint++; // Update the node data.
        ...
        updateText.call(_this); // Continue running the simulation, at the current timepoint only.
    ...
    function eventList() {
      ...
      inputs.on('click', function (d) {
        ...
        }), 1);else fdg.settings.eventChangeCount.push(this.textContent);
      });
      return {
        container: container,
        inputs: inputs
    ...
    function addOrbits() {
      ...
      var orbits = this.svg.selectAll('circle.orbit').data(this.metadata.event.filter(function (event) {
    ...
    function color$1() {
      ...
      this.colorLegend = this.legends.append('div').classed('fdg-legend fdg-legend__color', true).classed('fdg-hidden', this.settings.eventChangeCountAesthetic !== 'color');
    ...
    function size() {
      ...
      this.sizeLegend = this.legends.append('div').classed('fdg-legend fdg-legend__size', true).classed('fdg-hidden', this.settings.eventChangeCountAesthetic !== 'size');
      var legendDimensions = [200, 100];
    ...
    function both() {
      ...
      this.bothLegend = this.legends.append('div').classed('fdg-legend fdg-legend__both', true).classed('fdg-hidden', this.settings.eventChangeCountAesthetic !== 'both');
      var legendDimensions = [200, 100];
    ...
    function collide(alpha) {
      ...
      return function (d) {
        var r = fdg.settings.eventChangeCountAesthetic !== 'color' ? d.r + fdg.settings.maxRadius + fdg.settings.padding : d.r + fdg.settings.minRadius + fdg.settings.padding;
        var nx1 = d.x - r;
        var nx2 = d.x + r;
        var ny1 = d.y - r;
        var ny2 = d.y + r;
