(function (global, factory) {
    ...
    var settings = {
      ...
      shapeBy: {
        ...
        variable: null,
        label: null,
        shapes: ['cicle', 'square', 'triangle', 'diamond']
      },
      shape: 'circle',
      // time settings
      timepoint: 0,
      ...
      if (this.settings.stratify) {
        ...
        nest.forEach(function (stratum, i) {
          ...
          stratum.nParticipants = metadata.id.filter(function (d) {
            return d.category === stratum.value;
          }).length;
    ...
    function color(metadata) {
      ...
      switch (colorBy.type) {
        case 'frequency':
          var range = colorBy.mirror ? d3["scheme".concat(colorBy.colorScheme)][colorBy.nColors].reverse() : d3["scheme".concat(colorBy.colorScheme)][colorBy.nColors];
        ...
        case 'categorical':
          scale = d3.scaleOrdinal().domain(domain).range(d3["scheme".concat(colorBy.colorScheme)]);
          break;
    ...
    function addLegends() {
      ...
      if (this.settings.colorBy.type === 'continuous') {
        ...
        var legendItems = _container.append('svg').attr('width', 200).attr('height', 20 * this.scales.color.domain().length).selectAll('g').data(this.scales.color.domain()).join('g');
    ...
    function addForceSimulation() {
      ...
      this.forceSimulation = d3.forceSimulation().nodes(this.data.nested).alphaDecay(0.01).velocityDecay(0.9).force('center', d3.forceCenter(this.settings.orbitRadius / 2, this.settings.height / 2)) // cleared after first interval
