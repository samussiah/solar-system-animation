(function (global, factory) {
    ...
    function draw(worker, color) {
      ...
      worker.onmessage = function (event) {
        ...
        var marks = g.selectAll('.fdg-static__mark').data(event.data.nodes).join(main.settings.shape === 'circle' ? 'circle' : 'rect').classed('fdg-static__mark', true).attr('fill', event.data.color).attr('fill-opacity', 0.25); //.attr('fill-opacity', 0);
        ...
        }).attr('width', event.data.radius * 2).attr('height', event.data.radius * 2); //marks
        //    .transition()
        //    .duration(5000)
    ...
    */
    ...
    function data() {
      ...
      this.data.nested.forEach(function (d) {
        ...
        d.value.stateIndex = d.value.group.findIndex(function (di) {
          return di === d.value.state;
        });
        ...
        d.value.timeSincePreviousState = _this.settings.timepoint - d.value.state.start_timepoint; // update coordinates when:
        //   - state changes
        //   &&
        //   (
        ...
        if (!(d.value.state.duration < 5 && d.value.timeSincePreviousState < 5 && d.value.statePrevious.event !== 'Home')) d.value.coordinates = {
          x: event.x,
          y: event.y
        };
    ...
    function update$2() {
      ...
      switch (true) {
        ...
        case /static/i.test(this.modalText):
          ...
          emphasizeComponent.call(this, this.containers.svgBackground.selectAll('.fdg-static__mark'), 'stroke', 'rgba(215,25,28,0)', 'rgba(215,25,28,.5)');
          break;
    ...
    function both(svg, legendDimensions) {
      ...
      var marks = svg.selectAll('circle.legend-mark').data(this.settings.colors()).join(this.settings.shape === 'circle' ? 'circle' : 'rect').classed('legend-mark', true).attr('fill', function (d) {
        return '#aaa';
      }).attr('fill-opacity', 0.5).attr('stroke', function (d) {
        return '#aaa';
      }).attr('stroke-opacity', 1);
      ...
      });else marks.attr('x', function (d, i) {
        return i * (legendDimensions[0] / _this.settings.colors().length) + legendDimensions[0] / _this.settings.colors().length / 2 - (i + _this.settings.minRadius);
      }).attr('y', function (d, i) {
    ...
    function addForceSimulation() {
      ...
      .force('charge', forceManyBodySampled().strength(this.settings.chargeStrength * 5)).force('collide', d3.forceCollide().radius(function (d) {
