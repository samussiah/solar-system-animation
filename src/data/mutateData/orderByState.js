    ...
    if (this.settings.stateChange === 'ordered' && this.metadata) {
        ...
        d3.nest()
            ...
            .rollup((group) => {
                ...
                group.forEach((d, i) => {
                    const currState = this.metadata.event.find((event) => event.key === d.event);
                    const prevState = this.metadata.event.find(
                        (event) => event.key === group[i - 1]?.event
                    );

                    if (i === 0) {
                        d.start_timepoint = 1;
                        d.end_timepoint =
                            d.start_timepoint + currState.allIds.findIndex((id) => id === d.id);
                    } else {
                        d.start_timepoint =
                            prevState.allIds.findIndex((id) => id === d.id) +
                            prevState.start_timepoint +
                            1;
                        d.end_timepoint =
                            currState.allIds.findIndex((id) => id === d.id) +
